...
class Cam_lane():
    ...
    def callback(self,data):
        ...
        ######## bird eye로 바꾸기
        # 좌하, 좌상, 우상, 우하
        # srcpints = np.float32(np.array([[width * 100/100, height * 100/100],[width * 100/100, height * 100/100],[width * 100/100, height * 100/100],[width * 100/100, height * 100/100]]))
        # dstponts = np.float32(np.array([[width * 100/100, height * 100/100],[width * 100/100, height * 100/100],[width * 100/100, height * 100/100],[width * 100/100, height * 100/100]]))
        ...
        mask = cv2.getPerspectiveTransform(src_points,dst_points) 
        image_warp = cv2.warpPerspective(cv2_image,mask,(width,height))
        image_warpgray = cv2.cvtColor(image_warp,cv2.COLOR_BGR2GRAY)
        image_warpblur = cv2.GaussianBlur(image_warpgray,(blurThre1,blurThre1),0)
        image_warpedge = cv2.Canny(np.uint8(image_warpblur),CanThre1,CanThre2)

        all_lines = cv2.HoughLinesP(image_warpedge,1,np.pi/180,Hough_Thre,Hough_MinLen,Hough_MinGap)

        # for line in all_lines:
        #     x1,y1, x2,y2 = line[0]
        #     cv2.line(image_warp,(x1,y1),(x2,y2),(0,255,0),3)
        ...
        for line in left_lines:  #파란색 차선
            ...
            x1,y1, x2,y2 = line[0]
            cv2.line(image_warp,(x1,y1),(x2,y2),(255,0,0),3)
        
        for line in right_lines: #빨간색 차선
            #line_img = cv2_image.copy()
            x1,y1, x2,y2 = line[0]
            cv2.line(image_warp,(x1,y1),(x2,y2),(0,0,255),3)
        ...
        if size == 0:
            ...
            x2 = int((720.0 - b_left)/m_left)
        cv2.line(image_warp,(x1,0),(x2,400+320),(0,255,0),2)
        ...
        if size == 0:  #### roi width의 최대값을 넣어야함 
            ...
            x2 = int((720.0 - b_right)/m_right)
        cv2.line(image_warp,(x1,0),(x2,400+320),(0,255,255),2)
        ...
        cv2.imshow("image_warpedge",image_warpedge)
