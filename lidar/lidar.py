...
class lidar():
    ...
    def callback(self, data):
        ...
        ########## lidar ##########
        angle_min_degree = data.angle_min / np.pi * 180
        angle_max_degree = data.angle_max / np.pi * 180    
        '''
        ...
        for i in range(self.roi_degree_offset + 1):
            left_roi_lidar.append([i, -i, data.ranges[i]]) ## origin index, roi angle, distance # 가운데 기준으로 왼쪽은 -angle로 지정
        j = self.roi_degree_offset
        for i in range(360 - self.roi_degree_offset, 360):            
            right_roi_lidar.append([i, j, data.ranges[i]]) ## origin index, roi angle, distance    # 가운데 기준으로 오른쪽은 +angle로 지정
            j -= 1
        
        roi_lidar = right_roi_lidar + left_roi_lidar  ## origin index, theta, angle, distance
        
        for i in range(len(roi_lidar)):
            if roi_lidar[i][2] > 1.5 :
                roi_lidar[i][2] = self.infinity
            print(f"{roi_lidar[i][1]}도의 index : {roi_lidar[i][0]}, 거리 : {roi_lidar[i][2]} ")
