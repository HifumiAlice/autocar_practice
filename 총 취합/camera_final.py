...
class Cam_lane():
    ...
    def stopline(self,cv2_image):
        ...
        if cv2.countNonZero(L_roi) > 10000:   ## 화면 내에 일정이상 흰색점이 발견됐다면 정지선이다.  전체 화면 픽셀수 대비 70% 이상이면 정지선으로 인식해도 될듯?   
    ...
    def linefinde(self,cv2_image):
        ...
        if self.line_find == "l":
            ...
            x_center = x_right  - 240#300   # -250은 sector1에서 나가짐 그 이상은 안나가지는 것 같음 -270은 들어올때 들어와짐
