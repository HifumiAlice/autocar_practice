...
class lidarClustering( ) :
    ...
    def clusteringCallback(self, _data) :
        ...
        if self.obs_exist_flag :
            ...
            rospy.loginfo("[clusteringCB] distance({}), groupedSpace={}".format(self.SEARCH_DISTANCE, groupedSpace))

            # 3. Analysis & publish
            self.analysis_pub(_data, filteredData, groupedData, groupedSpace)
        else :
            self.empty_data_pub()
