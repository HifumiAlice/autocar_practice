...
def data(msg):
    ...
    add_roi = roi_ranges_left +  roi_ranges_right
    ...
    #for i in range(0,roi_degree_offset * 2):
    #    print("{}번째 거리 : {}".format(i,add_roi[i]))
        


    ########## lidar를 통해 간단하게 장애물 만나면 정지 없으면 직진하는 코드 짜기 ##########
    count = 0
    for i in range(0,roi_degree_offset):
        if roi_ranges_left[i] < 2.0:
            print("왼쪽 {}번쨰 거리 : {}".format(i,roi_ranges_left[i]))
            count += 1
        if roi_ranges_right[i] < 2.0:
            print("오른쪽 {}번쨰 거리 : {}".format(i,roi_ranges_right[i]))
            count += 1
